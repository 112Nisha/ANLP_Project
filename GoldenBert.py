...
class DiscourseDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        encoding = self.tokenizer.encode_plus(sent1, sent2, add_special_tokens=True, max_length=self.max_length, padding='max_length', truncation=True, return_attention_mask=True, return_tensors='pt')
...
def model_initializer(device):
    ...
    model.load_state_dict(torch.load('/kaggle/input/bert_2_epochs/pytorch/default/1/epoch_2.pt'))
    model.to(device)
...
def predict_discourse_marker(model, tokenizer, sentence1, sentence2, device):
    model.eval()
    encoding = tokenizer.encode_plus(sentence1, sentence2, add_special_tokens=True, max_length=MAX_LENGTH, padding='max_length', truncation=True, return_attention_mask=True, return_tensors='pt')

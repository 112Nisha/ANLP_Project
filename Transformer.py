...
from params import EMBEDDING_DIM, NUM_HEADS, NUM_DECODERS, FF_DIM, DROPOUT_RATE

def causal_masking(size):
    mask = torch.triu(torch.full((size, size), float('-inf')), diagonal=1)
    mask = mask.masked_fill(mask == float('-inf'), float(0.0))
    return mask
...
class Transformer(nn.Module):
    ...
    def forward(self, input_seq, target=None):
        ...
        if target is not None:
            ...
            output = self.decoder(input_seq, input_seq)
            _ , attention_weights = self._decode_with_attention(input_seq, input_seq, target_mask=None)
            attention_weights_tensor = torch.stack(attention_weights, dim=0)
            batch_attention_weights = attention_weights_tensor.split(1, dim=1)
            batch_attention_weights_list = [batch.squeeze(0).squeeze(0) for batch in batch_attention_weights]
